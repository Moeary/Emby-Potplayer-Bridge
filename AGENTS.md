...
- `defaultPlayer` 是默认播放目标，只允许 `potplayer` 或 `web`；旧版只有 `enabled` 时按兼容规则迁移：`enabled=true` → `potplayer`，`enabled=false` → `web`。
- 详情页左侧原始“播放”按钮执行默认目标；右侧“一次性选择”按钮必须执行另一目标，不修改默认设置。默认网页时右侧应为“在 PotPlayer 中播放”，默认 PotPlayer 时右侧应为“在网页中播放”。
- 一次性选择必须保留单项、播放全部、随机播放的模式和准确的条目 ID，不能因 SPA 节点重建、缓存或异步解析而播放相邻集数。
