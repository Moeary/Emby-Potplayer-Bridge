...
- 原始入口旁固定显示一个小型连体双图标按钮组：左侧使用当前 Emby/Jellyfin 图标，右侧使用 PotPlayer 图标；悬停或键盘聚焦时显示详细提示。两者是单次辅助入口，不修改默认设置，也不因默认状态改变目标。
- 双按钮组必须保留单项、播放全部、随机播放的模式和准确的条目 ID，不能因 SPA 节点重建、缓存或异步解析而播放相邻集数。
- 普通视频卡片的 `data-action="play"`、覆盖层和列表项播放也必须遵循 `defaultPlayer`；若页面没有把条目 ID 放在 DOM，须用当前目录与卡片名称反查，不能因缺少 ID 直接放行网页播放器。
