...
默认方式继续使用已有 `enabled` 设置。`enabled=false` 代表默认网页播放；内容脚本发出的 `destination=potplayer` 仅对当前请求覆盖默认值，不写入设置。后台同时验证发送页面的来源是否获准。网页选择同步点击原按钮并仅放行该次事件；新选择或页面跳转会取消尚未完成的旧解析请求。
