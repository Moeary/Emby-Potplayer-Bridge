...
详情页和列表播放控件会保留原始播放按钮，并在右侧显示固定的小型连体双图标按钮：原始按钮遵循 `defaultPlayer`（兼容旧版 `enabled`）决定默认走网页或 PotPlayer；辅助组左侧显示当前 Emby/Jellyfin 图标，右侧显示 PotPlayer 图标，鼠标悬停或键盘聚焦时展示详细提示，两个辅助入口仅本次生效且不修改默认设置。普通视频卡片的播放覆盖层交给 Emby/Jellyfin 网页处理，不会被默认 PotPlayer 自动接管；后台仍验证发送页面的来源是否获准。新的播放请求或页面跳转会取消尚未完成的旧解析请求。
