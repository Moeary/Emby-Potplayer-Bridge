...
在 Emby 或 Jellyfin 网页中点击播放时，原始播放按钮遵循扩展中保存的默认播放器设置；按钮旁还提供网页和 Windows 本机 PotPlayer 两个临时入口。项目由 Chrome 扩展和一个本地 Native Host 组成，当前版本为 `v0.8.3`。

## 快速开始

### 直接下载 Release

已发布版本见 [Releases](https://github.com/Moeary/Emby-Potplayer-Bridge/releases)。`v0.8.3` 的发布包命名如下；尚未发布时请从源码安装：

- `PotPlayerBridgeHost-0.8.3.exe`：本地播放桥程序。
- `EmbyPotPlayerBridge-0.8.3-extension.zip`：Chrome 扩展，解压后用于“加载已解压的扩展”。
- `config.toml`：配置模板。
...
- 未获准的站点不显示这些扩展入口；普通视频卡片、单项、播放全部和随机播放会保留各自的播放模式。
