...
在 Emby 或 Jellyfin 网页中点击播放时，原始播放按钮遵循扩展中保存的默认播放器设置；按钮旁还提供网页和 Windows 本机 PotPlayer 两个临时入口。项目由 Chrome 扩展和一个本地 Native Host 组成，当前版本为 `v0.8.4`。

## 快速开始

### 直接下载 Release

已发布版本见 [Releases](https://github.com/Moeary/Emby-Potplayer-Bridge/releases)。`v0.8.4` 的发布包命名如下；尚未发布时请从源码安装：

- `PotPlayerBridgeHost-0.8.4.exe`：本地播放桥程序。
- `EmbyPotPlayerBridge-0.8.4-extension.zip`：Chrome 扩展，解压后用于“加载已解压的扩展”。
- `config.toml`：配置模板。
...
详情页和列表工具栏保留站点原本的播放按钮，并在旁边显示一个小型连体双图标按钮组：

- 原本的“播放/恢复播放”按钮：遵循插件设置中的默认播放器。勾选“默认使用 PotPlayer 播放”时进入 PotPlayer，取消时放行网页播放器。
- 左侧显示当前站点的 Emby 或 Jellyfin 图标，右侧显示 PotPlayer 图标；鼠标悬停或键盘聚焦时显示对应的详细提示。
- 两个图标入口仍分别执行网页播放和 PotPlayer 播放，仅本次生效，不修改默认设置。
- 未获准的站点不显示这些扩展入口；普通视频卡片的播放覆盖层交给网页播放器，详情页和列表工具栏的单项、播放全部、随机播放仍保留各自的默认/临时播放模式。
